...
class SolarPanel:
    ...
    def __init__(
        ...
        self.time_degradation_yearly = params["time_degradation_yearly"]
        self.temperature_degradation_per_deg = params["temperature_degradation_per_deg"]

    def compute_power_generated_variable_cells(
        self,
        eclipse_status: bool,
        attitude: int,
        ...
        delta_t: TimeDelta,
        t: int
    ) -> Tuple[Quantity["power"], Quantity[""]]:
        """
        Calculate the power generated by the solar panels and return the current efficiency.
        ...
        Parameters:
            ...
            mode (int): The spacecraft's operating mode.
            delta_t (TimeDelta): Time step for the update.
            t (int): Current time step.

        Returns:
            Tuple[Quantity["power"], Quantity[""]]: 
                - Power generated by the solar panels [W]
                - Current solar cell efficiency [dimensionless]
        """
        ...
        if not eclipse_status:
            power = (
                self.get_effective_surface(attitude, r_earth_sun, r_earth_sat, gs_coords)
                * self.solar_flux
                * eff
            )
            return power.to(u.W), eff
        else:
            return 0 * u.W, eff
    
    def solar_cells_efficiency(self, delta_t: TimeDelta, temperature : float, t : int) -> float:
        """
        Calculate the solar panel efficiency considering degradation over time and temperature effects.

        Parameters:
            delta_t (TimeDelta): Time step for the update.
            temperature (float): Current temperature of the solar cells.
            t (int): Current time step.
            
        Returns:
            float: The current efficiency of the solar cells.
        """
        
        # Time-based degradation
        delta_years = delta_t.to(u.year).value  # float, unitless

        # Time-based degradation factor
        time_degradation_factor = 1 - (self.time_degradation_yearly * delta_years*t)

        # Temperature-based degradation
        temperature_degradation = self.temperature_degradation_per_deg * (temperature - 20)

        efficiency = self.panels_efficiency * time_degradation_factor * (1 - temperature_degradation)

        # Store the result if needed
        self.solar_cells_efficiency_last_step = efficiency

        return efficiency * u.dimensionless_unscaled
...
class Eps(SubSystem):
    ...
    def __init__(
        ...
        self.solar_cells_efficiency_last_step = self.solar_panels.panels_efficiency * u.dimensionless_unscaled
    ...
    def update_batteries(
        ...
        #         eclipse_status, attitude, r_earth_sun, r_earth_sat, gs_coords, mode
        #     )
        ...
        if self.battery_level < self.max_battery:
            power_generated, efficiency = self.solar_panels.compute_power_generated_variable_cells(
                eclipse_status, attitude, r_earth_sun, r_earth_sat, gs_coords, mode, delta_t, t
            )
            self.battery_level += power_generated * delta_t
            self.power_generation_last_step = power_generated
            self.solar_cells_efficiency_last_step = efficiency * u.dimensionless_unscaled
        else:
            self.power_generation_last_step = 0.0 * (u.W)
    ...
    def get_solar_cells_efficiency(self) -> Quantity[""]:
        return self.solar_cells_efficiency_last_step
