...
class SolarPanel:
    ...
    def compute_power_generated(
        ...
        """
        hardcode = False
        # hardcode = True  # hardcoded: uncomment to simulate some scenario depending on the mode for experiment
        if hardcode:
            if (
                mode == 1
            ):  # to simulate no power generation in safe mode (report case study)
                return 0 * u.W
