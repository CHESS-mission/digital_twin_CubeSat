...
class ModeSwitch:
    ...
    def switch_mode(self, eps:Eps, telecom:Telecom, com_window:bool, eclipse_status:bool, measurement_session: bool, safe_flag = False) -> None:
        if safe_flag:
            self.operating_mode = "SAFE"
        else:
            match self.operating_mode:
                case "IDLE":
                    self.mode_switch_from_IDLE(eps, telecom, com_window, eclipse_status, measurement_session)
                case "SAFE":
                    self.mode_switch_from_SAFE()
                case "CHARGING":
                    self.mode_switch_from_CHARGING(eps, telecom, com_window, eclipse_status, measurement_session)
                case "UHF_COM":
                ...
                case _:
                    raise AssertionError(f"The current operating mode ({self.operating_mode}) does not exist")
    ...
    def mode_switch_from_IDLE(self, eps:Eps, telecom:Telecom, com_window:bool, eclipse_status:bool, measurement_session: bool):
        ...
        if not (eclipse_status) and (eps.battery_level <= eps.min_battery):
            ...
            # Else try to measure
            if (eps.battery_level >= eps.measure_threshold) and (not telecom.data_storage_full) and (measurement_session):
    ...
    def mode_switch_from_CHARGING(self, eps:Eps, telecom:Telecom, com_window:bool, eclipse_status:bool, measurement_session:bool):
        # Try to measure
        if (eps.battery_level >= eps.measure_threshold) and (not telecom.data_storage_full) and (measurement_session):
            self.operating_mode = "MEASUREMENT"
            # eps.MeasurementDuration = 0
    ...
    def mode_switch_from_UHF_COM(self, eps:Eps, telecom:Telecom, com_window:bool):
        ...
        if (eps.battery_level >= eps.xb_threshold) and (telecom.handshake) and (com_window) and (not telecom.downlink_complete):
    ...
    def mode_switch_from_MEASUREMENT(self, eps:Eps, com_window:bool, eclipse_status:bool):
        pass

    def mode_switch_from_MEASUREMENT(self, eps:Eps, telecom:Telecom, com_window:bool, eclipse_status:bool, dt:float):
